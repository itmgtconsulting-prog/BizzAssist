...
export async function GET(request: NextRequest) {
  return withCronMonitor(
    { jobName: 'purge-unverified-users', schedule: '0 5 * * *', intervalMinutes: 1440 },
    async () => {
      ...
      try {
        ...
        const cutoff = new Date(
          Date.now() - VERIFICATION_WINDOW_HOURS * 60 * 60 * 1000
        ).toISOString();
        ...
        const unverified = users.filter(
          (u) =>
            !u.email_confirmed_at && u.created_at && new Date(u.created_at).toISOString() < cutoff
        );
