...
export default function VerifiedLinks({
  ...
  const fetchLinks = useCallback(async () => {
    ...
    try {
       ...
       * @param alts - Alternative URLs for platformen
      ...
      const merged: LinkItem[] = Array.isArray(rawLinks)
        ? rawLinks
            ...
            .map((raw) => {
              ...
              const baseUrl = raw.isAutoGenerated ? (aiUrl ?? supabaseUrl ?? raw.url) : raw.url;
              const isResolved = raw.isAutoGenerated && !!(aiUrl ?? supabaseUrl);
              const alts = mergedAlts[raw.platform] ?? [];
              // Byg altCounts opslag
              const altCounts: Record<string, number> = {};
              for (const alt of alts) {
                altCounts[alt] = verMap.get(alt)?.verified_count ?? 0;
              }
      ...
      if (aiSocials) {
        for (const [platform, url] of Object.entries(aiSocials)) {
          ...
          const best = pickBestUrl(url, alts);
          merged.push({
            platform,
            label: meta.label,
            icon: meta.icon,
    ...
    <div className="w-full overflow-hidden rounded-lg border border-slate-700/50">
      ...
      <div className="divide-y divide-slate-700/30">
        {links.map((link, i) => {
          ...
          // Filtrer same-base-domain alts — spejl logikken fra handleOpenAltPopup
