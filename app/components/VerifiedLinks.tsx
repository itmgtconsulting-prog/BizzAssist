...
interface LinkItem {
  ...
  color: string;
  url: string;
  /** Confidence score fra AI-søgning (0-100) — undefined hvis ikke AI-fundet */
  confidence?: number;
  ...
  /** Alternative URLs for denne platform med confidence metadata */
  alternatives: AISocialEntry[];
  /** Verificerings-counts per alternativ URL */
  altCounts: Record<string, number>;
}

/** Et socialt medie-link med confidence metadata fra AI-søgning */
interface AISocialEntry {
  url: string;
  /** Confidence score 0-100 fra Claude */
  confidence: number;
  /** Begrundelse fra Claude (valgfrit) */
  reason?: string;
}
...
interface VerifiedLinksProps {
  ...
  entityName: string;
  lang: 'da' | 'en';
  /** AI-fundne sociale medier-URLs med confidence — overstyrer auto-genererede søgelinks */
...
interface AltPopupState {
  platform: string;
  /** Filtrerede alternativer med confidence metadata (allerede over tærskel) */
  alternatives: AISocialEntry[];
  /** Verificerings-counts per alternativ URL */
  altCounts?: Record<string, number>;
}
          ...
          {popup.alternatives.map((alt, i) => {
            ...
            return (
              <a
                ...
                className="flex items-center justify-between gap-2 px-3 py-2 hover:bg-slate-700/30 transition-colors group"
              >
                <div className="flex flex-col min-w-0 flex-1">
                  <span className="text-[11px] text-slate-300 group-hover:text-blue-300 truncate transition-colors">
                    {hostname}
                  </span>
                  <span className="text-[9px] text-slate-400 truncate">{url}</span>
                  {alt.reason && (
                    <span className="text-[9px] text-slate-400 truncate mt-0.5">{alt.reason}</span>
                  )}
                </div>
                <div className="flex items-center gap-1.5 flex-shrink-0">
                  {count > 0 && <span className="text-[10px] text-emerald-400">👍 {count}</span>}
                  {/* Confidence badge */}
                  <span className={`px-1 py-0.5 rounded text-[8px] font-medium ${confBadgeClass}`}>
                    {alt.confidence}%
                  </span>
                  <ExternalLink
                    size={11}
...
export default function VerifiedLinks({
  ...
  const fetchLinks = useCallback(async () => {
    ...
    try {
      ...
      // Supabase returnerer string[] — konverteres til AISocialEntry[] uden confidence
      const supabaseAltsRaw: Record<string, string[]> = alternativesRes.ok
        ? await alternativesRes.json()
        : {};
      const mergedAlts: Record<string, AISocialEntry[]> = {};
      for (const [platform, urls] of Object.entries(supabaseAltsRaw)) {
        mergedAlts[platform] = urls.map((url) => ({ url, confidence: 80 }));
      }
      // AI-alternativer med confidence overstyrer Supabase-data
      if (aiAlternatives) {
      ...
      /**
       * Vælger den URL (primær eller alternativ) med flest verificeringer for en platform.
       * Ved uafgjort (inkl. 0) bruges AI's primære URL.
       *
       * @param primaryUrl - AI's primære URL
       * @param alts - Alternative entries med URL og confidence for platformen
       * @returns Den URL med højest verified_count, og dens counts
       */
      function pickBestUrl(
        ...
        const primaryVer = verMap.get(primaryUrl);
        let best = {
          url: primaryUrl,
          verifiedCount: primaryVer?.verified_count ?? 0,
          rejectedCount: primaryVer?.rejected_count ?? 0,
          userVerdict: primaryVer?.user_verdict ?? null,
        };
        for (const alt of alts) {
          const altVer = verMap.get(alt.url);
          if ((altVer?.verified_count ?? 0) > best.verifiedCount) {
            best = {
              url: alt.url,
              verifiedCount: altVer?.verified_count ?? 0,
              rejectedCount: altVer?.rejected_count ?? 0,
              userVerdict: altVer?.user_verdict ?? null,
            };
          }
        }
        return best;
      ...
      const merged: LinkItem[] = Array.isArray(rawLinks)
        ? rawLinks
            .filter((raw) => {
              ...
              const aiEntry = aiSocials?.[raw.platform];
              if (aiEntry?.url) return true;
              return platformVerifiedUrl.has(raw.platform);
            })
            .map((raw) => {
              const aiEntry = aiSocials?.[raw.platform];
              const supabaseUrl = platformVerifiedUrl.get(raw.platform);
              const baseUrl = raw.isAutoGenerated
                ? (aiEntry?.url ?? supabaseUrl ?? raw.url)
                : raw.url;
              const isResolved = raw.isAutoGenerated && !!(aiEntry?.url ?? supabaseUrl);
              const alts = mergedAlts[raw.platform] ?? [];
              // Byg altCounts opslag (keyed by URL string)
              const altCounts: Record<string, number> = {};
              for (const alt of alts) {
                altCounts[alt.url] = verMap.get(alt.url)?.verified_count ?? 0;
              }
              // Auto-vælg URL med flest verificeringer
              const best = pickBestUrl(baseUrl, alts);
              return {
                ...
                verifiedCount: best.verifiedCount,
                rejectedCount: best.rejectedCount,
                userVerdict: best.userVerdict,
      ...
      if (aiSocials) {
        for (const [platform, entry] of Object.entries(aiSocials)) {
          if (!entry?.url) continue;
          if (merged.some((l) => l.platform === platform)) continue;
          const meta = PLATFORM_META[platform];
          if (!meta) continue;
          const alts = mergedAlts[platform] ?? [];
          const altCounts: Record<string, number> = {};
          for (const alt of alts) {
            altCounts[alt.url] = verMap.get(alt.url)?.verified_count ?? 0;
          }
          const best = pickBestUrl(entry.url, alts);
          merged.push({
            platform,
            label: meta.label,
            icon: meta.icon,
   ...
   * Inkluderer verificerings-counts per alternativ URL til visning i popup.
   *
   * @param link - Linket hvis alternativer skal vises
   */
  const handleOpenAltPopup = (link: LinkItem) => {
    if (popupPlatform === link.platform) {
      setAltPopup(null);
      setPopupPlatform(null);
    } else {
      // Filtrer alternativer: fjern same-base-domain og dem under confidence-tærskel
      const filteredAlts = link.alternatives.filter(
        (alt) =>
          alt.url !== link.url &&
          !isSameBaseDomain(alt.url, link.url) &&
    ...
    <div className="w-full overflow-hidden rounded-lg border border-slate-700/50">
      ...
      <div className="divide-y divide-slate-700/30">
        {links.map((link, i) => {
          ...
          // Filtrer same-base-domain alts og dem under confidence-tærskel
          const filteredAltsCount = link.alternatives.filter(
            (alt) =>
              alt.url !== link.url &&
              !isSameBaseDomain(alt.url, link.url) &&
              alt.confidence >= confidenceThreshold
          ...
          // Vis fuld URL (uden https://www.) for AI-fundne links der ikke er søgelinks
          let displayUrl: string | undefined;
          if (!link.isAutoGenerated) {
            try {
              const parsed = new URL(link.url);
              const host = parsed.hostname.replace(/^www\./, '');
              // Inkludér sti hvis den ikke er tom/rod
          ...
          return (
            ...
            >
              ...
              >
                ...
                {/* Confidence badge — kun for AI-fundne links med confidence-data */}
                {link.confidence !== undefined && !link.isAutoGenerated && (
                  <span
                    className={`flex-shrink-0 px-1 py-0.5 rounded text-[8px] font-medium ${confBadgeClass}`}
                    title={`AI confidence: ${link.confidence}%`}
                  >
                    {link.confidence}%
                  </span>
                )}
                ...
                <ExternalLink
                  size={9}
                  className="text-slate-400 group-hover:text-blue-400 flex-shrink-0 ml-0.5"
                />
              </a>

              {/* Alt.-kolonne — skjult helt hvis ingen reelle alternativer */}
                ...
                {hasAlts && (
                  <button
                    onClick={(e) => {
                      e.preventDefault();
                      e.stopPropagation();
                      if (hasAlts && !link.isAutoGenerated) {
                        handleOpenAltPopup(link);
                      }
                    }}
                    disabled={!hasAlts || link.isAutoGenerated}
                    title={
                      link.isAutoGenerated
                        ? lang === 'da'
                          ? 'Kør AI-søgning for at aktivere dette'
                          : 'Run AI search to enable this'
                        : `${filteredAltsCount} alternative links`
                    }
                    className={`flex items-center gap-0.5 min-w-[32px] min-h-[28px] px-1.5 rounded-md border transition-all text-[10px] font-medium
                      ${link.isAutoGenerated ? 'opacity-20 cursor-not-allowed border-transparent text-slate-600' : ''}
                      ${
                        hasAlts && !link.isAutoGenerated && isPopupOpen
                          ? 'bg-blue-600/20 border-blue-600/40 text-blue-400'
                          : hasAlts && !link.isAutoGenerated
                            ? 'border-transparent text-slate-400 hover:text-blue-400 hover:bg-blue-500/15 hover:border-blue-500/30'
                            : ''
                      }`}
                  >
                    Alt.
                    {hasAlts && !link.isAutoGenerated && (
                      <ChevronDown size={9} className={isPopupOpen ? 'rotate-180' : ''} />
                    )}
                  </button>
                )}
              ...
              <div className="flex items-center justify-center px-3">
                <button
                  ...
                  disabled={isPending || !isAuthenticated || link.isAutoGenerated}
                  title={
                    link.isAutoGenerated
                      ? lang === 'da'
                        ? 'Kør AI-søgning for at aktivere dette'
                  ...
                  className={`flex items-center gap-1 min-w-[36px] min-h-[28px] px-1.5 rounded-md border transition-all text-xs font-medium
                    ...
                    ${link.isAutoGenerated ? 'opacity-20 cursor-not-allowed border-transparent text-slate-600' : ''}
                    ${!isAuthenticated && !link.isAutoGenerated ? 'opacity-30 cursor-not-allowed border-transparent text-slate-600' : ''}
                    ${
                      isUserVerified && isAuthenticated && !link.isAutoGenerated
                        ? 'bg-emerald-500/20 border-emerald-500/40 text-emerald-400'
                        : isAuthenticated && !link.isAutoGenerated
                          ? 'border-transparent text-slate-400 hover:text-emerald-400 hover:bg-emerald-500/15 hover:border-emerald-500/30'
                          : ''
                    }`}
                >
              ...
              <div className="flex items-center justify-center px-2">
                <button
                  ...
                  className={`flex items-center gap-1 min-w-[36px] min-h-[28px] px-1.5 rounded-md border transition-all text-xs font-medium
                    ...
                    ${!isAuthenticated && !link.isAutoGenerated ? 'opacity-30 cursor-not-allowed border-transparent text-slate-600' : ''}
                    ${
                      isUserRejected && isAuthenticated && !link.isAutoGenerated
                        ? 'bg-red-500/20 border-red-500/40 text-red-400'
                        : isAuthenticated && !link.isAutoGenerated
                          ? 'border-transparent text-slate-400 hover:text-red-400 hover:bg-red-500/15 hover:border-red-500/30'
                          : ''
                    }`}
                >
      ...
      {!isAuthenticated && (
        <div className="px-3 py-2 bg-slate-800/30 border-t border-slate-700/30 text-[10px] text-slate-400 text-center">
