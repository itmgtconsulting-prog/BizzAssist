 ...
 *
 * Tabel-layout med kolonner: Platform | Link | 👍 count | 👎 count.
...
import { ExternalLink, Loader2, ThumbsUp, ThumbsDown } from 'lucide-react';
...
export default function VerifiedLinks({
  ...
  /**
   * Henter links fra /api/links (platform-liste) og merger med:
   ...
   * Re-kører når aiSocials ankommer.
   */
  const fetchLinks = useCallback(async () => {
    ...
    try {
      ...
      const [linksRes, verificationsRes] = await Promise.all([
      ...
      // Disse er AI-fundne URLs der blev verificeret i en tidligere session.
      // Bruges til at gendanne links selv når AI-søgning ikke er kørt i denne session.
      ...
      const merged: LinkItem[] = Array.isArray(rawLinks)
        ? rawLinks
            // Skjul auto-genererede søgelinks — vis kun reelle DB-links og AI-fundne URLs.
            // Auto-genererede links vises også hvis de har en verificeret URL i Supabase-historik.
            .filter((raw) => {
              if (!raw.isAutoGenerated) return true;
              // Virkdk-linket er altid nyttigt (direkte CVR-link, ikke en søge-URL)
              if (raw.platform === 'virk') return true;
              const aiUrl = aiSocials?.[raw.platform as keyof AISocials];
              if (aiUrl) return true;
              // Vis også hvis platformen har en verificeret URL fra en tidligere AI-søgning
              return platformVerifiedUrl.has(raw.platform);
            })
            .map((raw) => {
              // Erstat auto-genereret URL med AI-fundet URL eller Supabase-verificeret URL
              const aiUrl = aiSocials?.[raw.platform as keyof AISocials];
      ...
      <div className="divide-y divide-slate-700/30">
        {links.map((link, i) => {
          ...
          return (
            <div
              key={`${link.platform}-${i}`}
              className={`grid grid-cols-[1fr_auto_auto] items-center px-3 py-2 transition-colors ${rowBg} ${isUserRejected ? 'opacity-60' : ''}`}
            >
              {/* Platform + link */}
              <a
                href={link.url}
