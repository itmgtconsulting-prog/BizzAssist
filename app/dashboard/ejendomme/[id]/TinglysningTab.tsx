...
export default function TinglysningTab({
  ...
  return (
    <div className="space-y-6">
      ...
      {/* BIZZ-1597: AI-ekstraktion af scannede akter */}
      {indskannedeAkter.length > 0 && bfe && (
        <AktExtractionButton bfe={bfe} aktNavn={indskannedeAkter[0].aktNavn} lang={lang} />
      )}
    </div>
  );
}
...
/**
 * BIZZ-1597: Knap til AI-ekstraktion af scannede akter.
 * Viser estimeret token-pris og kører ekstraktion on-click.
 */
function AktExtractionButton({
  bfe,
  aktNavn,
  lang,
}: {
    ...
    return (
      <div className="mt-4 bg-emerald-500/5 border border-emerald-500/20 rounded-xl p-4">
        <div className="flex items-center gap-2 mb-2">
          <Landmark className="w-4 h-4 text-emerald-400" aria-hidden />
        ...
        <p className="text-emerald-300 text-xs">
          {da
            ? `Fandt ${result.handler} handler, ${result.haeftelser} hæftelser, ${result.servitutter} servitutter fra scannet akt.`
            : `Found ${result.handler} transactions, ${result.haeftelser} mortgages, ${result.servitutter} easements from scanned deed.`}
          {result.tokensUsed > 0 && (
  ...
  return (
    <div className="mt-4">
      <button
        onClick={extract}
        disabled={loading}
        className="flex items-center gap-2 px-4 py-2.5 bg-emerald-600/20 hover:bg-emerald-600/30 border border-emerald-500/30 text-emerald-300 text-sm rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
      >
        {loading ? (
          <>
          ...
          <>
            ...
            {da
              ? 'Udtræk historisk data fra scannet akt'
              : 'Extract historical data from scanned deed'}
