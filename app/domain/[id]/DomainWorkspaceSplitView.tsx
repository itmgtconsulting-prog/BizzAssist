/**
 * DomainWorkspaceSplitView — 4-quadrant working surface der åbner når en
 * bruger vælger en sag fra Domain/Sager-tab.
 *
 * BIZZ-800: Toppen af siden (sidebar, topbar, Domain-header, tabs) bevares
 * uændret. Kun Sager-tab-content skifter fra "list" til denne split.
 *
 * Layout:
 *
 *   ┌───────────────────────────────┬───────────────────────────┐
 *   │ TOP-LEFT                      │ TOP-RIGHT                 │
 *   │   Sager-listen (altid synlig, │   AI-agent chat           │
 *   │   valgt row highlightet)      │   skriv hvad der skal ske │
 *   │───────────── horizontal ─────│─────── horizontal ────────│
 *   │ BOTTOM-LEFT                   │ BOTTOM-RIGHT              │
 *   │   Valgt sag-detail            │   Skabelon-valg for sagen │
 *   │   (dokumenter, noter, osv.)   │   tik kryds til at vælge  │
 *   └───────────────────────────────┴───────────────────────────┘
 *                        ↑ vertical divider ↑
 *
 * 3 resizable dividers — vertikal (venstre/højre bredde) + 2 horizontale
 * (top/bund-højde per kolonne). Alle positioner persistes i localStorage.
 *
...
export function DomainWorkspaceSplitView({
  ...
  // 3 splits: vertikal (venstre-kolonne bredde) + 2 horizontale (top-højde
  // per kolonne). Alle er persistet i localStorage.
  ...
  useEffect(() => {
    ...
    const lw = Number(window.localStorage.getItem(STORAGE.leftW));
    if (lw >= 25 && lw <= 80) setLeftWidthPct(lw);
  ...
  /** Generic drag handler factory. axis=x updates left-width, axis=y updates top-height. */
  ...
  // Column refs for scoped drag (horizontal dividers only move within their column)
  const leftColRef = useRef<HTMLDivElement>(null);
  const rightColRef = useRef<HTMLDivElement>(null);

  const selectedCase = cases.find((c) => c.id === selectedCaseId) ?? null;

  // ─── Case detail state (compact inline) ────────────────────────────────
  ...
  const sendAi = async () => {
    ...
    try {
      ...
      const json = (await r.json()) as { output_path?: string; generation_id?: string };
      setAiMessages((prev) => [
        ...prev,
        {
          role: 'assistant',
          text: json.output_path
            ? da
              ...
              : `✓ Document generated. Download it from the case documents tab.`
            : da
              ? '✓ Generation startet.'
              : '✓ Generation started.',
          timestamp: Date.now(),
  ...
  return (
    <div
      ref={containerRef}
      className="flex w-full border border-slate-700/40 rounded-xl overflow-hidden"
      style={{ height: 'calc(100vh - 280px)', minHeight: 480 }}
    >
      {/* LEFT COLUMN — sager + selected case */}
      <div ref={leftColRef} className="flex flex-col min-w-0" style={{ width: `${leftWidthPct}%` }}>
          ...
          <div className="p-3 space-y-3">
            {selectedCase && (
              <>
                ...
                {/* Dokumenter med checkbox-valg til AI-context */}
                <div>
      ...
      <div
        ...
        className="group w-1.5 shrink-0 cursor-col-resize bg-slate-800/40 hover:bg-blue-500/40 relative transition-colors"
      ...
      {/* RIGHT COLUMN — AI chat + templates */}
      <div ref={rightColRef} className="flex flex-col min-w-0 flex-1">
        {/* TOP-RIGHT: AI chat */}
        <div
                ...
                <ol className="list-decimal list-inside space-y-1 pl-1">
                  ...
                  <li>
                    {da
                      ...
                      : 'Select relevant documents from the case (left)'}
                  </li>
                  <li>
                    {da
          ...
          <div className="p-2 space-y-1">
            {templatesLoading ? (
              ...
              <p className="text-xs text-slate-500 py-4 text-center">
                {da ? 'Ingen aktive skabeloner i dette domain.' : 'No active templates.'}
              </p>
            ) : (
              templates.map((t) => {
