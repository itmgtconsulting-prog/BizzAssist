#!/usr/bin/env node
/**
 * BIZZ-1712: Fast Ejerskifte-only backfill (no handel lookups).
 * Handelsoplysninger are backfilled separately via backfill-ejf-handel-bulk.mjs.
 *
 * Usage:
 *   node scripts/backfill-ejf-ejerskifte-fast.mjs --env=prod [--after=CURSOR]
 */
...
async function main() {
  ...
  console.log(`BIZZ-1712: Fast Ejerskifte backfill (no handel lookups) — env=${TARGET_ENV}`);
    ...
    const data = await dfQuery(`{
      EJF_Ejerskifte(first: ${PAGE_SIZE}, virkningstid: "${virkningstid}"${afterClause}) {
        nodes {
          ...
          handelsoplysningerLokalId virkningFra virkningTil status registreringFra registreringTil
        }
  ...
  if (buf.length > 0) {
    const values = buf.map(n => `(${esc(n.id_lokalId)}, ${n.bestemtFastEjendomBFENr}, ${esc(n.overdragelsesmaade)}, ${n.overtagelsesdato ? esc(n.overtagelsesdato) : 'NULL'}, ${esc(n.handelsoplysningerLokalId)}, ${n.virkningFra ? esc(n.virkningFra) : 'NULL'}, ${n.virkningTil ? esc(n.virkningTil) : 'NULL'}, ${esc(n.status)}, ${n.registreringFra ? esc(n.registreringFra) : 'NULL'}, ${n.registreringTil ? esc(n.registreringTil) : 'NULL'}, now())`).join(',\n');
    const r = await runSql(`INSERT INTO ejf_ejerskifte (id_lokal_id, bfe_nummer, overdragelsesmaade, overtagelsesdato, handelsoplysninger_lokal_id, virkning_fra, virkning_til, status, registrering_fra, registrering_til, sidst_opdateret) VALUES ${values} ON CONFLICT (id_lokal_id) DO NOTHING`);
