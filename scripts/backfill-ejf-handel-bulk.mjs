#!/usr/bin/env node
/**
 * BIZZ-1709: Bulk backfill ejf_handelsoplysninger via cursor pagination.
 * Faster than per-ejerskifte lookup — fetches ALL handelsoplysninger directly.
 *
 * Usage:
 *   node scripts/backfill-ejf-handel-bulk.mjs --env=prod [--after=CURSOR]
 */
...
async function dfQuery(query, retries = 3) {
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      return data.data;
    } catch (e) { if (attempt < retries) { await new Promise(r => setTimeout(r, 3000 * (attempt + 1))); continue; } return null; }
  }
  return null;
}
...
async function main() {
  ...
  console.log(`BIZZ-1709: Bulk backfill EJF Handelsoplysninger — env=${TARGET_ENV}`);
  ...
  while (true) {
    ...
    const data = await dfQuery(`{
      EJF_Handelsoplysninger(first: ${PAGE_SIZE}, virkningstid: "${virkningstid}"${afterClause}) {
        nodes {
    ...
    const nodes = data.EJF_Handelsoplysninger?.nodes ?? [];
    const pageInfo = data.EJF_Handelsoplysninger?.pageInfo;
  ...
  // Final flush
  if (buf.length > 0) {
    const values = buf.map(n => `(${esc(n.id_lokalId)}, ${n.kontantKoebesum ?? 'NULL'}, ${n.samletKoebesum ?? 'NULL'}, ${n.loesoeresum ?? 'NULL'}, ${n.entreprisesum ?? 'NULL'}, ${n.koebsaftaleDato ? esc(n.koebsaftaleDato) : 'NULL'}, ${esc(n.valutakode)}, ${n.virkningFra ? esc(n.virkningFra) : 'NULL'}, ${n.virkningTil ? esc(n.virkningTil) : 'NULL'}, ${esc(n.status)}, ${n.registreringFra ? esc(n.registreringFra) : 'NULL'}, ${n.registreringTil ? esc(n.registreringTil) : 'NULL'}, now())`).join(',\n');
    const r = await runSql(`INSERT INTO ejf_handelsoplysninger (id_lokal_id, kontant_koebesum, samlet_koebesum, loesoeresum, entreprisesum, koebsaftale_dato, valutakode, virkning_fra, virkning_til, status, registrering_fra, registrering_til, sidst_opdateret) VALUES ${values} ON CONFLICT (id_lokal_id) DO NOTHING`);
