...
for (const k of ['BIZZ-635','BIZZ-636','BIZZ-637','BIZZ-638','BIZZ-639']) {
  const cm = await r('GET',`/rest/api/3/issue/${k}/comment`);
  const already = (JSON.parse(cm.body).comments||[]).some(c => /Playwright-verifikation|API-level verifikation|Code-level verifikation/.test(flat(c.body).join('')));
  if (already) { console.log(`\n=== ${k}: already verified, SKIP ===`); continue; }
  const d = await r('GET',`/rest/api/3/issue/${k}?fields=summary,description`);
  const di = JSON.parse(d.body);
  console.log(`\n=== ${k}: ${di.fields.summary} ===`);
  console.log(flat(di.fields.description).join('').slice(0,1400));
}
