...
const tickets = [
  // ── EPIC ─────────────────────────────────────────────────────────────────
  {
    ...
    description: [
      p(
        'Comprehensive optimization of BizzAssist data fetching layer based on full audit of all 80+ API routes and 18+ external services.'
      ),
      heading('Goals'),
      bulletList([
      ...
      heading('Scope'),
      p(
        'This epic covers analysis and implementation across: Datafordeler (BBR, MAT, DAR, VUR, EJF), CVR ElasticSearch, Tinglysning mTLS, EMO energy labels, and all cron/background jobs. No changes to UI components — backend API layer only.'
      ),
      heading('Risk Mitigation'),
      p(
        'All changes must maintain existing functionality. Each ticket should be implemented with feature flags or gradual rollout where applicable. Existing cache TTLs must be preserved unless explicitly changed.'
      ),
    ],
  },
  ...
  // ── TICKET 1: Centralize OAuth token cache ─────────────────────────────
  {
    ...
    description: [
      ...
      heading('Impact'),
      p(
        'Reduces auth.datafordeler.dk calls from 4x to 1x on concurrent page loads. Lower latency on first request of each cold-start cycle.'
      ),
  ...
  // ── TICKET 2: Add cache headers to CVR endpoints ──────────────────────
  {
    ...
    description: [
      heading('Problem'),
      p(
        'Several high-traffic CVR endpoints have NO HTTP cache headers, causing every client-side call to hit the server and ElasticSearch on every tab switch or page navigation. Company data changes infrequently (daily at most).'
      ),
      ...
      heading('Impact'),
      p(
        'Eliminates redundant ElasticSearch queries on tab navigation and back-button usage. Estimated 40-60% reduction in CVR ES calls for repeat visits within 1 hour.'
      ),
...
User clicks owner link:
  [Call 9] GET /api/cvr-public?enhedsNummer=XXX (same data fetched again)`,
      ...
      heading('Impact'),
      p(
        'Eliminates 3-15 redundant API calls per company page load depending on ownership depth. Faster initial render for companies with complex ownership structures.'
      ),
      labelValue('Effort', 'M (3-5 days)'),
      labelValue(
        'Risk',
        'Medium — must ensure owner chain depth is still correctly resolved for deep structures. Test with companies that have 4+ levels of corporate ownership.'
      ),
    ],
  },
  ...
  // ── TICKET 4: Parallelize fetchBFENummer ──────────────────────────────
  {
    ...
    description: [
      heading('Problem'),
      p(
        'The server-side BBR prefetch (fetchBbrForAddress → fetchBFENummer in app/lib/fetchBbrData.ts) performs 3 sequential API calls that could be partially parallelized. This function is the single biggest latency bottleneck on the property detail page (200-400ms typical).'
      ),
  ...
  Promise.all([
    ...
    Step 3: Vurderingsportalen ES → ejerlejlighed BFE
      ...
      heading('Impact'),
      p(
        'Reduces fetchBFENummer latency by ~100-200ms (the duration of the slower of steps 2/3). This directly improves property page server-side render time.'
      ),
      labelValue('Effort', 'S (1-2 days)'),
      labelValue(
        'Risk',
        'Low — Steps 2 and 3 have no data dependency on each other. Fallback logic must be preserved.'
      ),
...
4. Background: GET /api/regnskab/xbrl?cvr=X&offset=0&limit=20  → RE-PARSE ALL 20 docs for cache`,
      ...
      heading('Impact'),
      p(
        'Eliminates redundant XBRL parsing on the server. For companies with 20+ years of financials, this saves significant server CPU time and reduces background network traffic.'
      ),
      labelValue('Effort', 'S (1-2 days)'),
  ...
  // ── TICKET 6: Consolidate EJF GraphQL queries ─────────────────────────
  {
    ...
    description: [
      heading('Problem'),
      p(
        'Two separate routes query the exact same Datafordeler GraphQL endpoint (flexibleCurrent/v1) with the same EJFCustom_EjerskabBegraenset query and ejendeVirksomhedCVRNr filter, duplicating work.'
      ),
      ...
      p(
        'When user clicks "Ejendomme" tab, both /api/ejendomme-by-owner and /api/salgshistorik/cvr are called. Both independently fetch the same list of BFE-numbers from Datafordeler before doing their respective work.'
      ),
      heading('Solution'),
      bulletList([
      ...
      heading('Impact'),
      p(
        'Eliminates 1 redundant Datafordeler GraphQL call per company property tab load. Reduces Datafordeler API quota usage.'
      ),
      labelValue('Effort', 'S (1-2 days)'),
  ...
  // ── TICKET 7: DAWA deprecation preparation ────────────────────────────
  {
    ...
    description: [
      heading('Context'),
      p(
        'DAWA (Danmarks Adresseregister) at api.dataforsyningen.dk is being deprecated on July 1, 2026. BizzAssist currently uses DAWA as both a primary source and a fallback throughout the codebase. DAR (via Datafordeler GraphQL) is already implemented as the primary for some routes, but DAWA is still heavily used.'
      ),
      ...
      heading('Deadline'),
      p(
        'DAWA shutdown: July 1, 2026. All DAWA dependencies must be removed before this date. Recommend completing by June 1, 2026 to allow testing buffer.'
      ),
      labelValue('Effort', 'L (1-2 weeks)'),
      labelValue(
        'Risk',
        'High if not completed before deadline — address resolution is critical path for property lookups. Medium risk during migration — must test all fallback scenarios.'
      ),
    ],
  },
  ...
  // ── TICKET 8: Property page request deduplication ─────────────────────
  {
    ...
    description: [
      heading('Problem'),
      p(
        'The property detail page (EjendomDetaljeClient.tsx, ~6800 lines) makes 12-15 independent API calls on load. While most are correctly parallelized, there is no client-side request deduplication. If a user navigates away and back, or if React re-renders the component, all calls fire again.'
      ),
  ...
  /api/cvr?vejnavn=...                (CVR ElasticSearch)`,
      ...
      heading('Impact'),
      p(
        'Eliminates redundant calls on React re-renders and tab navigation. Particularly valuable for users who switch between property tabs frequently.'
      ),
      labelValue('Effort', 'M (3-5 days)'),
      labelValue(
        'Risk',
        'Medium — touching the data layer of a large component requires careful regression testing'
      ),
    ],
  },
...
EMO SOAP: 100-200ms`,
      ...
      heading('Deliverable'),
      p(
        'Performance comparison document with recommendations for switching access methods where beneficial. Include latency measurements, quota implications, and migration effort estimates.'
      ),
  ...
  // ── TICKET 10: Tinglysning waterfall optimization ─────────────────────
  {
    ...
    description: [
      heading('Problem'),
      p(
        'Tinglysning data is loaded in a 2-step waterfall on the property detail page. The main tinglysning call must complete before the summarisk (summary) calls can start, because the UUID comes from the main response.'
      ),
      heading('Current Flow'),
      codeBlock(
...
Step 2 (after Step 1): Promise.all([
  ...
  → Latency: 100-200ms`,
      ...
      heading('Impact'),
      p(
        'Saves 100-300ms on tinglysning data display by eliminating the client→server→tinglysning round-trip for Step 2.'
      ),
      labelValue('Effort', 'S (1-2 days)'),
  ...
  // ── TICKET 11: CVR endpoint consolidation ─────────────────────────────
  {
    ...
    description: [
      heading('Problem'),
      p(
        'BizzAssist has multiple CVR endpoints that query the same ElasticSearch cluster (distribution.virk.dk) with overlapping functionality:'
      ),
      bulletList([
      ...
      heading('Impact'),
      p(
        'Reduced maintenance surface, consistent response shapes, easier caching strategy. No performance impact until endpoints are actually consolidated.'
      ),
      labelValue('Effort', 'M (3-5 days)'),
  ...
  // ── TICKET 12: BBR event-driven cache invalidation ────────────────────
  {
    ...
    description: [
      heading('Context'),
      p(
        'BizzAssist already has a cron job (/api/cron/pull-bbr-events) that polls the Datafordeler Hændelsesbesked API every 6 hours for building/unit change events. Currently this data is stored but not used for cache invalidation.'
      ),
      heading('Opportunity'),
      bulletList([
      ...
      heading('Impact'),
      p(
        'Could reduce BBR Datafordeler API calls by 80-90% while maintaining data freshness. Cache hit rate increases from ~50% (1h TTL) to ~95% (24h TTL with event invalidation).'
      ),
      labelValue('Effort', 'M (3-5 days)'),
      labelValue(
        'Risk',
        'Medium — must ensure Hændelsesbesked coverage is complete; fallback to TTL-based invalidation needed'
      ),
    ],
  },
];
