...
// BIZZ-2248: læs env fra process.env (GitHub Actions) med fallback til .env.local
// (dev-server-kompat under overgangen). Fjerner SPOF: samme script kører nu både
// på dev-serveren OG som redundant GitHub Actions-cron. De koordinerer via
// heartbeat-gating (en trigger opdaterer heartbeat → den anden ser jobbet friskt
// og springer over → ingen dobbelt-fyring).
let envContent = '';
try {
  envContent = fs.readFileSync('/root/BizzAssist/.env.local', 'utf8');
} catch {
  /* ingen .env.local (fx GitHub Actions) — brug process.env */
}
function env(key) {
