...
const d = await r('GET','/rest/api/3/issue/BIZZ-641?fields=summary,description,status,priority,labels');
const di = JSON.parse(d.body);
console.log(`=== ${di.key}: ${di.fields?.summary}`);
console.log(`Status: ${di.fields?.status?.name}  Priority: ${di.fields?.priority?.name}  Labels: ${(di.fields?.labels||[]).join(', ')}`);
console.log('---');
console.log(flat(di.fields?.description).join(''));
