...
function table(headers, rows, widths) {
  ...
  const dataRows = rows.map((row, ri) =>
    new TableRow({
      children: row.map((v, ci) => cell(v, { bg: ri % 2 === 0 ? 'ffffff' : 'f8fafc', width: widths[ci] })),
    })
  );
  return new Table({
...
function checkTable(rows) {
  ...
  const dataRows = rows.map(([scenario, result], ri) =>
    new TableRow({
      children: [
        cell(scenario, { bg: ri % 2 === 0 ? 'ffffff' : 'f8fafc', width: 75 }),
        new TableCell({
          children: [new Paragraph({
            children: [new TextRun({ text: result, size: 18, bold: true, color: '16a34a', font: 'Calibri' })],
          })],
    ...
    children: [
      ...
      new Paragraph({
        children: [new TextRun({
          ...
          bold: true, size: 40, color: DARK, font: 'Calibri',
        })],
      ...
      p('BizzAssist er en dansk business intelligence-platform, der aggregerer og præsenterer offentligt tilgængeligt data om faste ejendomme, virksomheder og personer for professionelle brugere — herunder ejendomsmæglere, advokater, revisorer, banker og erhvervsinvestorer.'),
      spacer(),
      p('Formålet med adgang til e-TL er at give BizzAssist-brugere direkte indsigt i tingbogens offentlige oplysninger som en integreret del af ejendoms- og virksomhedsanalyserne:'),
      b('Ejendomsvisning: Vis tinglyste oplysninger (ejere, adkomsthavere, hæftelser, servitutter, pantebreve) direkte i ejendomsdetaljesiden for en fast ejendom identificeret ved BFE-nummer.'),
      b('Virksomhedsvisning: Vis registrerede hæftelser i Personbogen (virksomhedspant, løsørepant, fordringspant, ejendomsforbehold) for virksomheder identificeret ved CVR-nummer.'),
      b('Dokumentadgang: Hent og præsenter tinglysningsdokumenter (pantebreve, skøder, servitutter) som PDF til brugere med behov for at se det fulde dokument.'),
      spacer(),
      p('Systemet anvender udelukkende forespørgsels-services — der anmeldes ikke dokumenter via systemadgangen.'),
      divider(),

      // 2. SERVICES
      h1('2. Implementerede services'),
      p('BizzAssist anvender HTTP API (forespørgsel) med 2-vejs SSL (OCES systemcertifikat). Følgende services er implementeret:'),
      ...
      b('Certifikatformat: PFX (PKCS#12), konfigureret som base64-encodet miljøvariabel på Vercel-hosting'),
      b('Anmeldelser: Ikke implementeret — systemet foretager udelukkende forespørgsler'),
      divider(),

      // 3. TEST
      h1('3. Gennemført testforløb'),
      p('Testforløbet er gennemført mod fællestestmiljøet (https://test.tinglysning.dk) med et NETS test-certifikat (devtest4-miljø).'),
      ...
      b('404 Not Found: Returnerer { error: "Ejendom ikke fundet i tingbogen" } med HTTP 404. UI viser informativ besked.'),
      b('500 / 502: Logges server-side, returneres som { error: "Ekstern API fejl" } uden interne detaljer.'),
      b('Andre 4xx/5xx: Propageres med status-kode og logges.'),

      h3('Netværk og timeout:'),
      b('Alle kald har AbortSignal.timeout(15000) (15 sekunder). Ved timeout destrueres socket og returneres fejlbesked.'),
      b('Netværksfejl fanges og returneres som HTTP 500 med generisk fejlbesked.'),

      h3('Certifikat-fejl:'),
      b('Hvis certifikats-miljøvariablerne ikke er sat, returneres HTTP 503 — systemet deaktiverer sig selv pænt.'),
      b('Certifikatet loades fra base64-encodet miljøvariabel (Vercel-kompatibelt) eller filsti som fallback.'),
      ...
      h3('Caching:'),
      b('Svar caches i 1 time (Cache-Control: public, s-maxage=3600) for at reducere belastningen på e-TL.'),
      divider(),
      ...
      b('Certifikat-opbevaring: Base64-encodet PFX i krypteret Vercel-miljøvariabel (NEMLOGIN_CERT_B64)'),
      spacer(),
      p('Alle kald til e-TL afsendes fra en dedikeret proxy med statisk IP-adresse. Nedenstående IP-adresse bedes tilføjes til e-TLs IP-whitelist:'),
      spacer(),
      table(
      ...
      spacer(),
      p('Bemærk: IP-adressen 93.161.46.78 (lokal udviklingsmaskine) er allerede whitelistet til testmiljøet og benyttes under udviklingstest.', { color: GRAY, italics: true }),
      spacer(),
      b('Systemcertifikat (produktion): OCES3 FOCES systemcertifikat udstedt til Pecunia IT Consulting ApS, CVR 44718502, via MitID Erhverv / Nets.'),
      divider(),

      // 5. STORKUNDE
      h1('5. Forudsætninger (storkunde-registrering)'),
      p('Vi er bekendt med, at adgang til produktionsmiljøet kræver registrering som storkunde hos SKAT i henhold til § 20, stk. 3 i bekendtgørelse nr. 1634 af 29. juni 2021 om tekniske krav og forskrifter for tinglysningssystemet.'),
      p('Status: Pecunia IT Consulting ApS er registreret til betaling af tinglysningsafgift, hvilket udgør grundlaget for storkunde-status. Der er ikke udstedt et separat certifikat i forbindelse med registreringen.', { bold: true }),
      p('Registreringen som storkunde bekræftes ved dokumentation for registrering til betaling af tinglysningsafgift, som vedlægges som bilag.'),
      divider(),
      ...
      // BILAG
      h1('Bilag'),
      b('1. Dokumentation for implementerede services (kode-eksempler på request/response-håndtering)'),
      b('2. Dokumentation for registrering til betaling af tinglysningsafgift (bekræftelse af storkunde-status hos SKAT)'),
      spacer(),
      spacer(),
      p('Pecunia IT Consulting ApS forbeholder sig retten til at videregive tingbogsdata til egne abonnenter alene inden for rammerne af offentlighedsprincippet og persondataforordningen (GDPR). Der videregives ikke data til tredjeparter uden for platformens brugerbase.', { color: GRAY, italics: true, size: 16 }),
    ],
  }],
});
