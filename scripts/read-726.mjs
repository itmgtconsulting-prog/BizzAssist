...
const r = await req('/rest/api/3/issue/BIZZ-726?fields=summary,description,comment');
const j = JSON.parse(r.body);
console.log(`BIZZ-726: ${j.fields.summary}\n${'─'.repeat(70)}`);
console.log(adf(j.fields.description).join('').trim().slice(0, 2000));
...
for (const c of comments.slice(-2)) {
  ...
  console.log(adf(c.body).join('').trim().slice(0, 1000));
}
