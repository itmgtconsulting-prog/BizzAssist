...
const r = await req('/rest/api/3/issue/BIZZ-731?fields=summary,description,comment');
const j = JSON.parse(r.body);
const text = adfToText(j.fields.description).join('').trim();
console.log(`BIZZ-731: ${j.fields.summary}\n${'─'.repeat(70)}\n${text.slice(0, 2000)}`);
if (text.length > 2000) console.log(`... (${text.length - 2000} mere)`);
