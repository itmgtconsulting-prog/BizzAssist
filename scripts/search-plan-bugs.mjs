...
const res = await r('POST','/rest/api/3/search/jql',{
  jql: 'project = BIZZ AND status != Done AND status != "On Hold" AND (summary ~ "plan" OR summary ~ "plankonfiguration" OR summary ~ "abonnement" OR summary ~ "subscription" OR labels in (billing, subscription, plan-configs, ai-tokens)) ORDER BY priority DESC, updated DESC',
  fields:['summary','status','priority','labels'], maxResults: 30
});
const d = JSON.parse(res.body);
console.log(`Plan/billing-relaterede åbne tickets: ${(d.issues||[]).length}\n`);
