-- ============================================================================
-- Migration 023: RLS Security Fixes
-- BIZZ-141: support_questions overly permissive RLS
-- BIZZ-142: audit_log.resource_type NOT NULL enforcement
-- BIZZ-143: notifications RLS tenant-scoping audit
-- BIZZ-144: property_snapshots RLS too permissive (USING true)
-- ============================================================================
-- NOTE: property_snapshots, notifications, and audit_log live in per-tenant
-- dynamic schemas (created by provision_tenant_schema). This migration:
--   a) Patches all existing tenant schemas via a DO block loop
--   b) Updates provision_tenant_schema so new tenants get the fixed policies
-- support_questions lives in the public schema and is handled directly.
-- ============================================================================


-- ── BIZZ-141: Fix support_questions RLS ──────────────────────────────────
-- The current policy (support_questions_service_all) uses USING(true), which
-- allows any authenticated user to read all support questions from all tenants.
-- Replace with a restrictive policy: only service_role may access this table.
-- Authenticated users have no legitimate reason to query this table directly.

ALTER TABLE public.support_questions ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "support_questions_service_all" ON public.support_questions;
DROP POLICY IF EXISTS "support_questions_select"       ON public.support_questions;
DROP POLICY IF EXISTS "Allow authenticated read"        ON public.support_questions;
DROP POLICY IF EXISTS "support_questions_own_tenant"    ON public.support_questions;

-- Deny all access to authenticated and anon roles.
-- Service role bypasses RLS by default in Supabase, so no explicit policy
-- is needed for service_role — it always has full access.
-- This effectively makes the table service-role-only, matching the original
-- intent stated in migration 008: "Only service role can insert".
CREATE POLICY "support_questions: deny authenticated"
...
-- ── BIZZ-142 / 143 / 144: Patch existing tenant schemas ─────────────────
-- Iterate all provisioned tenant schemas and apply fixes in-place.
...
BEGIN
  ...
  LOOP
    ...
    -- ── BIZZ-142: audit_log.resource_type NOT NULL ──────────────────────
    -- The column was defined NOT NULL in provision_tenant_schema (migration 005),
    -- but any rows inserted before that fix may have NULLs. Backfill them.
    EXECUTE format(
      'UPDATE %I.audit_log SET resource_type = ''unknown'' WHERE resource_type IS NULL',
      r.schema_name
    );
    -- Enforce the constraint going forward (idempotent if already NOT NULL).
    EXECUTE format(
      'ALTER TABLE %I.audit_log ALTER COLUMN resource_type SET NOT NULL',
      r.schema_name
    );

    -- ── BIZZ-144: Tighten property_snapshots policies ───────────────────
    -- Original policies used USING(true), granting any authenticated user
    -- read/write access across all tenants. Replace with tenant-member checks.
    EXECUTE format(
      'DROP POLICY IF EXISTS "property_snapshots: service read"  ON %I.property_snapshots',
      r.schema_name
    );
    EXECUTE format(
      'DROP POLICY IF EXISTS "property_snapshots: service write" ON %I.property_snapshots',
      r.schema_name
    );
    EXECUTE format(
      'DROP POLICY IF EXISTS "property_snapshots: members read"  ON %I.property_snapshots',
      r.schema_name
    );
    EXECUTE format(
      'DROP POLICY IF EXISTS "property_snapshots: members write" ON %I.property_snapshots',
      r.schema_name
    );
    EXECUTE format(
      'DROP POLICY IF EXISTS "property_snapshots: admin delete"  ON %I.property_snapshots',
      r.schema_name
    );

    EXECUTE format(
      'ALTER TABLE %I.property_snapshots ENABLE ROW LEVEL SECURITY',
      r.schema_name
    );
    -- Read: tenant members only
    EXECUTE format(
      'CREATE POLICY "property_snapshots: members read" ON %I.property_snapshots '
      'FOR SELECT USING (public.is_tenant_member(tenant_id))',
      r.schema_name
    );
    -- Write (INSERT/UPDATE): service_role only — cron job uses service key
    EXECUTE format(
      'CREATE POLICY "property_snapshots: service write" ON %I.property_snapshots '
      'FOR INSERT WITH CHECK (true)',
      r.schema_name
    );
    EXECUTE format(
      'CREATE POLICY "property_snapshots: service update" ON %I.property_snapshots '
      'FOR UPDATE USING (true)',
      r.schema_name
    );
    -- Delete: tenant admin only
    EXECUTE format(
      'CREATE POLICY "property_snapshots: admin delete" ON %I.property_snapshots '
      'FOR DELETE USING (public.is_tenant_admin(tenant_id))',
      r.schema_name
    );

    -- ── BIZZ-143: Audit notifications RLS ──────────────────────────────
    -- Migration 007 already creates correct tenant-scoped policies for new
    -- tenants. For tenants provisioned before 007 was applied, the policies
    -- may be missing. Drop and recreate to ensure consistency.
    EXECUTE format(
      'DROP POLICY IF EXISTS "notifications: members read"   ON %I.notifications',
      r.schema_name
    );
    EXECUTE format(
      'DROP POLICY IF EXISTS "notifications: members update" ON %I.notifications',
      r.schema_name
    );
    EXECUTE format(
      'DROP POLICY IF EXISTS "notifications: service write"  ON %I.notifications',
      r.schema_name
    );
    EXECUTE format(
      'DROP POLICY IF EXISTS "notifications: admin delete"   ON %I.notifications',
      r.schema_name
    );

    EXECUTE format(
      'ALTER TABLE %I.notifications ENABLE ROW LEVEL SECURITY',
      r.schema_name
    );
    EXECUTE format(
      'CREATE POLICY "notifications: members read" ON %I.notifications '
      'FOR SELECT USING (public.is_tenant_member(tenant_id))',
      r.schema_name
    );
    EXECUTE format(
      'CREATE POLICY "notifications: members update" ON %I.notifications '
      'FOR UPDATE USING (public.is_tenant_member(tenant_id)) WITH CHECK (public.is_tenant_member(tenant_id))',
      r.schema_name
    );
    -- Notifications are written by the cron job via service_role
    EXECUTE format(
      'CREATE POLICY "notifications: service write" ON %I.notifications '
      'FOR INSERT WITH CHECK (true)',
      r.schema_name
    );
    EXECUTE format(
      'CREATE POLICY "notifications: admin delete" ON %I.notifications '
      'FOR DELETE USING (public.is_tenant_admin(tenant_id))',
      r.schema_name
    );
...
-- ── Update provision_tenant_schema: bake fixes in for future tenants ─────
-- Recreate the function so that all newly provisioned tenants receive:
--   - Tenant-member-scoped property_snapshots policies (not USING true)
--   - audit_log.resource_type defined NOT NULL from creation
--   - Correct notifications policies (already correct in 007, kept here for
--     completeness and to match the patched existing-tenant logic above)
...
BEGIN
  ...
  -- ── 8. audit_log ─────────────────────────────────────────
  -- BIZZ-142: resource_type is explicitly NOT NULL (enforced from creation)
  ...
  -- BIZZ-144: Policies are now tenant-member-scoped (not USING true)
  ...
  -- Tenant members may read their own snapshots
  EXECUTE format(
    'CREATE POLICY "property_snapshots: members read" ON %I.property_snapshots FOR SELECT USING (public.is_tenant_member(tenant_id))',
    p_schema_name
  );
  -- Snapshot writes are performed by the cron job using service_role (bypasses RLS),
  -- but we add an explicit INSERT policy for defence-in-depth
  EXECUTE format(
    'CREATE POLICY "property_snapshots: service write" ON %I.property_snapshots FOR INSERT WITH CHECK (true)',
    p_schema_name
  );
  EXECUTE format(
    'CREATE POLICY "property_snapshots: service update" ON %I.property_snapshots FOR UPDATE USING (true)',
    p_schema_name
  );
  -- Only tenant admins may delete snapshots
  EXECUTE format(
    'CREATE POLICY "property_snapshots: admin delete" ON %I.property_snapshots FOR DELETE USING (public.is_tenant_admin(tenant_id))',
    p_schema_name
  );
  ...
  -- ── 11. notifications ─────────────────────────────────────
  -- BIZZ-143: Tenant-scoped RLS (correct since migration 007)
