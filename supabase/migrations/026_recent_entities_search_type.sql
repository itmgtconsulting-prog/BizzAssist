-- ============================================================
-- Migration 026: Add 'search' to recent_entities entity_type constraint
-- BizzAssist — Fix tag bar: search-type recents were rejected by DB
-- ============================================================
-- The original CHECK constraint on recent_entities only allows
-- ('company','property','person'). The app also stores 'search' entries
-- (one per entity visited, with resultType/resultHref in entity_data)
-- so the RecentEntityTagBar can show recent tags.
-- All 'search' inserts were silently rejected at DB level — hence no tags.
-- ============================================================
...
BEGIN
  ...
  LOOP
    -- Drop the auto-named inline check constraint
    EXECUTE format(
      'ALTER TABLE %I.recent_entities DROP CONSTRAINT IF EXISTS recent_entities_entity_type_check',
      r.schema_name
    );
    -- Add updated constraint including 'search'
    EXECUTE format(
      'ALTER TABLE %I.recent_entities ADD CONSTRAINT recent_entities_entity_type_check '
      'CHECK (entity_type IN (''company'',''property'',''person'',''search''))',
      r.schema_name
    );
  END LOOP;
END;
...
-- ── 2. Update provision_tenant_schema for new tenants ────────────────────
-- Replace only the recent_entities CREATE TABLE snippet inside the function
-- by recreating the full function body with the updated constraint.
-- The full provision_tenant_schema is in migration 007; we only patch it here.
...
BEGIN
  ...
  -- ── 11. recent_entities ──────────────────────────────────
  -- NOTE: entity_type includes 'search' for tracking visited-entity tag bar
...
-- Restore permission restrictions
